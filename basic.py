...
def check_haarcascadefile():
    if not os.path.isfile("haarcascade_frontalface_default.xml"):
        messagebox.showerror(
            "Error", "Missing haarcascade file!\nDownload from OpenCV GitHub."
        )
        window.destroy()
...
def TakeImages():
    ...
    progress_label = tk.Label(
        progress_window, text="Capturing...", bg="#2e2e2e", fg="white"
    )
    ...
    while True:
        ...
        for x, y, w, h in faces:
            ...
            cv2.putText(
                img,
                f"Captured: {sampleNum}/{total_samples}",
                (x, y - 10),
    ...
    if sampleNum > 0:
        ...
        messagebox.showinfo(
            "Success", f"Successfully captured {sampleNum} images for {name}"
        )
...
def TrainImages():
    ...
    tk.Label(
        train_window,
        text=f"Training on {len(faces)} images...",
        bg="#2e2e2e",
        fg="white",
    ).pack(pady=20)
    train_window.update()
    ...
    log_activity(
        "TRAINING", f"Model trained with {len(faces)} images for {unique_ids} students"
    )
...
def TrackImages():
    ...
    while True:
        ...
        cv2.putText(
            img,
            f"Marked Today: {len(attendance_marked)}",
            (10, 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
        ...
        for x, y, w, h, Id, conf in current_results:
            ...
            if conf < CONFIG["confidence_threshold"]:
                ...
                if len(student) > 0:
                    ...
                    department = (
                        str(student["Department"].values[0])
                    ...
                    # Check cooldown
                    if (
                        Id not in attendance_marked
                        or (current_time - attendance_marked[Id])
                        > CONFIG["attendance_cooldown"]
                    ):
                        ...
                        with open(attendance_file, "a", newline="") as f:
                            writer = csv.writer(f)
                            writer.writerow(
                                [Id, name, department, date, timeStamp, confidence]
                            )
    ...
    messagebox.showinfo(
        "Attendance Complete",
        f"Total attendance marked today: {len(attendance_marked)}",
    )
    status_label.config(
        text=f"Attendance complete: {len(attendance_marked)} students", fg="green"
    )
...
def ViewAttendance():
    ...
    tk.Label(date_frame, text="Select Date:", bg="#1e1e1e", fg="white").pack(
        side=tk.LEFT, padx=5
    )
    ...
    tk.Button(
        date_frame, text="Load", command=load_attendance, bg="#4CAF50", fg="white"
...
def ViewStudents():
    ...
    if not os.path.isfile("StudentDetails/StudentDetails.csv"):
        tk.Label(
            student_window, text="No students registered", bg="#1e1e1e", fg="white"
        ).pack(pady=50)
        return
    ...
    tk.Label(
        student_window, text=f"Total Students: {len(df)}", bg="#1e1e1e", fg="cyan"
    ).pack(pady=5)
...
def GenerateReport():
    ...
    tk.Label(
        report_window,
        text="Attendance Report Generator",
        bg="#1e1e1e",
        fg="white",
        font=("Arial", 14, "bold"),
    ).pack(pady=10)
    ...
    tk.Label(frame, text="From Date (DD-MM-YYYY):", bg="#2e2e2e", fg="white").grid(
        row=0, column=0, pady=5
    )
    from_date = tk.Entry(frame)
    from_date.grid(row=0, column=1, pady=5)

    tk.Label(frame, text="To Date (DD-MM-YYYY):", bg="#2e2e2e", fg="white").grid(
    ...
    def generate():
        ...
        summary = (
            combined.groupby(["ID", "Name"]).size().reset_index(name="Days Present")
        )
        ...
        for _, row in summary.iterrows():
            report_text.insert(
                tk.END,
                f"  {row['Name']} (ID: {row['ID']}): {row['Days Present']} days\n",
            )
...
def OpenSettings():
    ...
    tk.Label(frame, text="Camera Index:", bg="#2e2e2e", fg="white").grid(
    ...
    tk.Label(frame, text="Sample Count:", bg="#2e2e2e", fg="white").grid(
    ...
    tk.Label(frame, text="Confidence Threshold:", bg="#2e2e2e", fg="white").grid(
    ...
    tk.Label(frame, text="Cooldown (seconds):", bg="#2e2e2e", fg="white").grid(
...
def AdminPanel():
    ...
    def change_password():
        ...
        if new_pass:
            CONFIG["users"]["admin"]["password"] = hashlib.sha256(
                new_pass.encode()
            ).hexdigest()
    ...
    tk.Button(
        admin_win,
        text="View Class Strength",
        command=view_strength,
        bg="#FF9800",
        fg="white",
        width=25,
    ).pack(pady=10)
...
def login_screen():
    ...
    tk.Button(
        login_win, text="Login", command=check_login, bg="#4CAF50", fg="white", width=15
    ).pack(pady=15)
...
# Adding More Line
user_label = tk.Label(
    header_frame, text="", bg="#0d47a1", fg="white", font=("Arial", 10)
)
user_label.pack()
...
# Row 1
tk.Label(
    input_frame, text="Student ID:", bg="#2e2e2e", fg="white", font=("Arial", 10)
...
tk.Button(
    input_frame, text="Clear", command=clear, bg="#f44336", fg="white", width=8
).grid(row=0, column=2, padx=5)

tk.Label(input_frame, text="Name:", bg="#2e2e2e", fg="white", font=("Arial", 10)).grid(
    row=0, column=3, padx=10, pady=8, sticky="e"
)
txt2 = tk.Entry(input_frame, width=25, font=("Arial", 11))
txt2.grid(row=0, column=4, padx=10, pady=8)
tk.Button(
    input_frame, text="Clear", command=clear2, bg="#f44336", fg="white", width=8
).grid(row=0, column=5, padx=5)

# Row 2
tk.Label(input_frame, text="Email:", bg="#2e2e2e", fg="white", font=("Arial", 10)).grid(
...
tk.Button(
    input_frame, text="Clear", command=clear3, bg="#f44336", fg="white", width=8
).grid(row=1, column=2, padx=5)

tk.Label(
    input_frame, text="Department:", bg="#2e2e2e", fg="white", font=("Arial", 10)
).grid(row=1, column=3, padx=10, pady=8, sticky="e")
txt4 = tk.Entry(input_frame, width=25, font=("Arial", 11))
txt4.grid(row=1, column=4, padx=10, pady=8)
tk.Button(
    input_frame, text="Clear", command=clear4, bg="#f44336", fg="white", width=8
...
status_label = tk.Label(
    status_frame, text="Ready", bg="#0d47a1", fg="white", font=("Arial", 10)
)
status_label.pack(side=tk.LEFT, padx=10, pady=5)

version_label = tk.Label(
    status_frame, text="v2.0", bg="#0d47a1", fg="white", font=("Arial", 10)
)
